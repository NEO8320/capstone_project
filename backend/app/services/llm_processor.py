...
class GPTService:
    ...
    async def classify(
        ...
        #
        # ★ 2026-04-19 수정 (세계/연예 버킷 비움 현상 해결):
        #   기존 프롬프트는 "내용을 우선하세요" 로 GPT에게 재분류 권한을 주었다.
        #   그 결과 '세계' 섹션 기사도 "한국 대학" 언급 있으면 '사회' 로 보내
        #   세계/연예 카테고리가 비어버렸다.
        #   → 힌트를 *기본 선택지* 로 두고, 내용이 "전혀 다른 분야" 일 때에만
        #     override 하도록 프롬프트 편향을 뒤집는다.
        input_text = f"{title} [SEP] {summary}"
        if hint_category:
            input_text += (
                f"\n\n[섹션 힌트]\n"
                f"이 기사는 네이버 뉴스의 '{hint_category}' 섹션에서 수집되었습니다.\n"
                f"※ 기본 분류를 '{hint_category}' 로 하되, 기사 본문이 해당 카테고리와\n"
                f"   **완전히 무관한 분야**일 때에만 다른 카테고리로 분류하세요.\n"
                f"   (예: '연예' 섹션 기사가 경제 통계 분석이라면 '경제' 로 분류 가능)\n"
                f"   애매하면 반드시 힌트 카테고리 '{hint_category}' 를 선택하세요."
            )
