...
# ★ 2026-04-19: 세계/연예/스포츠는 크롤링 키워드("해외/연예인/스포츠")가
#   명확해서, GPT 가 본문 근거로 다른 카테고리로 옮겨도 크롤러 분류를 유지한다.
#   (키워드 신호가 강해 sticky 가 타당)
#
# ★ 2026-05-25 축소(중요): IT·과학/생활·문화 를 sticky 에서 제거했다.
#   이유 — 두 카테고리의 크롤링 키워드("IT", "문화")가 너무 넓어서
#   정치(문화체육관광부)·경제(문화산업)·사회(문화행사) 기사가 대량 딸려오는데,
#   GPT 는 이를 정확히 정치/경제/사회로 분류한다. 그런데 sticky 가 그 정확한
#   분류를 무시하고 전부 생활·문화/IT·과학 으로 욱여넣어, '정치 기사가 생활 탭에'
#   박히는 오분류가 발생했다. 빈 탭 문제는 서버사이드 카테고리 필터링으로 이미
#   해결됐으므로, 이 두 카테고리는 GPT 의 정확한 분류를 신뢰한다.
